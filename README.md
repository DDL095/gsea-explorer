...
`gsealens-explorer` 是一个**有状态、追问驱动**的 GSEA 富集结果探索框架。与"一次性把 CSV 丢给 LLM 解读"不同,它:
...
完整状态合约见 [`gsealens-explorer.agent.md`](gsealens-explorer.agent.md)。

## 为什么不直接问 ChatGPT?

| 一次性 LLM                                  | gsealens-explorer                                          |
| ------------------------------------------- | ---------------------------------------------------------- |
...
```mermaid
flowchart LR
    A[1. 你有一份<br/>GSEA 结果 RDS] --> B[2. 触发 gsealens-explorer skill]
    B --> C[3. 回答 5 个实验背景问题]
    C --> D[4. 选 1-3 个待验证假设]
...
```powershell
# 1. clone 仓库
git clone https://github.com/DDL095/gsealens-explorer.git D:\src\gsealens-explorer

# 2. 运行 deploy 脚本 (Windows)
powershell -ExecutionPolicy Bypass -File D:\src\gsealens-explorer\deploy\deploy_to_copilot.ps1

# 或 Linux/macOS
bash D:\src\gsealens-explorer\deploy\deploy_to_copilot.sh
```

部署目标默认是 `$env:USERPROFILE\.copilot\skills\gsealens-explorer\`(Windows)或
`~/.copilot/skills/gsealens-explorer/`(Unix)。个人化的实验案例与本地路径可放在
`<target>/.local_overrides/`,deploy 脚本不会覆盖它。

### 方式 B: 作为独立 Agent 角色定义使用

把 [`gsealens-explorer.agent.md`](gsealens-explorer.agent.md) 复制到你的 agent 框架能加载的位置,
...
├── gsealens-explorer.agent.md         Agent 角色 / 阶段合约
├── gsealens-explorer.md               紧凑角色概述
├── agents/                        子 agent 角色定义
...
### |NES| 富集方向框架 (|NES| enrichment direction)
...
flowchart LR
    ...
    Capsule --> Explorer["gsealens-explorer<br/>(本项目)"]
...
> gsealens-explorer 核心只依赖 R + Python + LLM runtime。如果某个 MCP 不可用,
> 对应的知识增广步骤会优雅降级。
...
```mermaid
flowchart LR
    Repo["本仓库 (公开, 脱敏)"] -->|deploy 脚本| Local["~/.copilot/skills/gsealens-explorer/"]
...
- **v0.6**: 测试套件 + [多 SA 并行架构 POC](docs/multi_sa_architecture.md)
- **v0.7**: 可视化代码落地
- **v1.0**: 测试覆盖 80%+ + 真实用户案例

## 引用

如果你在研究中使用 gsealens-explorer,请按 [`CITATION.cff`](CITATION.cff) 引用。
